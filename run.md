...
| `DB_USER` | The administrative username for the database. | `postgres` |
...
npm run start:dev
```
*(For production runtime, execute: `npm run start:prod`)*
...
npm run test
```
*(To execute tests with a full coverage report, use: `npm run test:cov`)*
...
```
*Note: The API will wait for the PostgreSQL container to pass its internal health checks before booting.*
